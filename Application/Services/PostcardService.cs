...
public class PostcardService : IPostcardService
{
  ...
  public async Task<UserPostcardDto> TransferPostcard(int postcardId, int userId)
  {
    UserPostcard userPostcard = await _userPostcardRepository.Get(postcardId);
    if (userPostcard == null)
    {
      throw new Exception("Postcard not found");
    }
    userPostcard.UserId = userId;
